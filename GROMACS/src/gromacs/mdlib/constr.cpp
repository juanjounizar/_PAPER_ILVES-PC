                              ...
                              ConstraintVariable        econq)
{
    ...
    if (lincsd != nullptr && !use_ilves_pc)
    {
        auto start = std::chrono::steady_clock::now();
        bOK = constrain_lincs(bLog || bEner, ir, step, lincsd, inverseMasses_, ir.shake_tol, cr, ms, x, xprime,
        ...
        constr_ms += std::chrono::duration_cast<std::chrono::microseconds>(end - start)
                          .count();
    ...
    if (shaked != nullptr && !use_ilves_pc)
    {
        auto start = std::chrono::steady_clock::now();
    ...
    if (use_ilves_pc) {
        if (econq != ConstraintVariable::Positions) {
            gmx_fatal(FARGS,
                        "Internal error, ILVES-PC called for constraining something else than "
                        "positions");
        }

        if (ir.efep != efepNO) {
            gmx_fatal(FARGS,
                        "Internal error, ILVES-PC does not support FEP");
        }
        ...
        auto start = std::chrono::steady_clock::now();
        bool bOK = ilves->solve(x.unpaddedArrayRef(),
                                       ...
                                       unsigned short* cFREEZE)
{
    ...
    if (ncon_tot > 0)
    {
        ...
        if (use_ilves_pc) {
            ...
            // TODO: We assume there is only one molecule with constraints,
            // and that its atoms are numbered contiguously starting in 0.

            /*
                * Compute the atomic graph from the list of bonds.
                */

            std::vector<std::set<int>> aux(numAtoms);
            std::set<int> atoms_in_mol;

            for (int bond = 0; bond < mol->n; ++bond) {
                // The bond connects atoms a and b.
                const int a = mol->bonds[2 * bond];
                const int b = mol->bonds[2 * bond + 1];

                // Atom a is connected to b.
                aux[a].insert(b);
                // Atom b is connected to a.
                aux[b].insert(a);

                atoms_in_mol.insert(a);
                atoms_in_mol.insert(b);
            }

            // The number of atoms in the molecule.
            mol->m = atoms_in_mol.size();

            mol->atomic_graph = (graph_t *)malloc(sizeof(graph_t));
            graph_init(mol->atomic_graph);

            mol->atomic_graph->m = mol->m;
            mol->atomic_graph->xadj = (int *)malloc((mol->m + 1) * sizeof(int));

            int adj_idx = 0;
            std::vector<int> adj;
            for (int atom = 0; atom < mol->m; ++atom) {
                // The atom is connected to itself.
                aux[atom].insert(atom);

                mol->atomic_graph->xadj[atom] = adj_idx;

                adj.insert(adj.end(),
                            aux[atom].begin(),
                            aux[atom].end());

                adj_idx += aux[atom].size();
            }
            mol->atomic_graph->xadj[mol->m] = adj_idx;

            mol->atomic_graph->adj = (int *)malloc(adj.size() * sizeof(int));

            // Copy adj into the atomic_graph adj.
            for (int i = 0; i < adj.size(); ++i) {
                mol->atomic_graph->adj[i] = adj[i];
            }
            ...
            // Get the element of each atom.
            mol->atoms = (element_t *) malloc(mol->m * sizeof(mol->atoms[0]));

            AtomIterator atoms(mtop);
            size_t i = 0;
            while (atoms->globalAtomNumber() < mtop.natoms && i < mol->m) {
                mol->atoms[i] = static_cast<element_t>(atoms->atom().atomnumber);
                atoms++;
                ++i;
            }
            ...
            // Init ILVES
            ilves = std::make_unique<IlvesPC>(mol.get(), omp_get_max_threads());
        }
    ...
    wcycle(wcycle_p)
{
    ...
    // Read the enviromental variable USE_ILVES_PC
    char *USE_ILVES_PC_ev = std::getenv("USE_ILVES_PC");
    if (USE_ILVES_PC_ev == NULL) {
        use_ilves_pc = false;
    }
    else {
        int USE_ILVES_PC_int = atoi(USE_ILVES_PC_ev);
        use_ilves_pc = (USE_ILVES_PC_int > 0);
    }
